...
def trade_loop():  
    ...
    if side != "hold" and mode == "Live":  
        ...
        log_container.info(f"🧪 [Paper Mode] Would {side.upper()} {qty} @ ${price:.2f}")

    return df, action_val, qty, side, sharpe, reason

# Button Actions
if st.session_state.test:  
    st.success("✅ Test Run OK — models and pipeline initialized.")
...
if st.session_state.sharpe:  
    ...
    sharpe_container.markdown(f"📉 **Sharpe Ratio:** `{sr:.4f}`")  
...
if st.session_state.run:  
    ...
    st.success("🔁 Bot is actively trading in the background.")
else:  
    ...
    try:
        ...
        if not positions:
            ...
            for p in positions:
                ...
                log_container.success(f"💸 Closed {side.upper()} position: {symbol} x{qty}")
    except Exception as e:
        log_container.error(f"❌ Cash Out Failed: {e}")
    st.session_state.cash = False
